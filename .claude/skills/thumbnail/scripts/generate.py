...
def main():
    ...
    est = sum(MODEL_CONFIG.get(c.get("model", model_default), cfg)["credits"] for c in concepts) * variants
    sys.stderr.write(f"[thumb] {len(concepts)} concept(s) × {variants} variant(s) = {total} images ≈ ${est:.2f} total\n")

    face_paths = list_face_refs()
    if not face_paths:
