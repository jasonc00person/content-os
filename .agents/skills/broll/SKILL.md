---
name: broll
description: "Generative B-roll inserts via Higgsfield. Codex reads the cut transcript + intent and proposes B-roll moments with cinematic prompts, you approve, the skill submits to Higgsfield, downloads, and overlays the clips on your video while keeping the voice audio. Triggers: add b-roll, broll, generate broll, insert b-roll, cinematic cutaways."
---

# B-Roll — Generative Cinematic Inserts via Higgsfield

The differentiator. Instead of keyword-matched stock footage, Codex reads your script's beats and writes **intentional generation prompts** that match what you're actually saying. You approve the manifest before spending money, then Higgsfield generates 5-second cinematic clips that overlay on your video at the right beats. Voiceover continues underneath.
...
```
1. Codex reads /tmp/video-editor/<job>/cuts.json + inbox/<job>/intent.md
2. Codex writes /tmp/video-editor/<job>/broll.json — proposed moments + prompts
...
python3 .Codex/skills/broll/scripts/generate.py /tmp/video-editor/<job>/broll.json

# Step 2: Overlay onto the video
bash .Codex/skills/broll/scripts/render.sh /tmp/video-editor/<job>/broll_resolved.json video-editor/outputs/<job>__9x16.mp4
```
...
- **Manifest review is mandatory** — never auto-approve. The prompts ARE the creative work; if Codex got one wrong, fix it before paying for it.
...
- Decide when to use B-roll vs let the talking head breathe — that's Codex's editorial call in the proposal step
