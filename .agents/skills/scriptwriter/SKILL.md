...
If no cached path: run `bash .Codex/skills/transcribe-url/scripts/transcribe-url.sh "<URL>"` first, then read the resulting file (with offset/limit if it's large).
