...
- **CTA rule (drives EVERYTHING about the caption shape):** scan the transcript for an explicit comment/DM CTA the creator actually said on camera (e.g. "comment SYSTEM", "DM me Codex", "drop AGREE in the comments").
...
For `customScheduled`, convert to UTC ISO 8601: `2026-04-20T19:00:00.000Z`. Confirm the user's timezone from AGENTS.md or ask if unknown. For vague "tomorrow" with no time, default to **noon local time** (convert to UTC accordingly).
...
- **0 matches** — surface that and stop (don't invent a row, don't silently skip). Filename → title mapping is usually obvious to the creator (e.g. `Codex content system.mp4` → "I Built the Ultimate Content System With Codex"); if Notion truly has nothing, the row probably needs to be created manually.
