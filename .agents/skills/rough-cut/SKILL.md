...
```bash
bash .Codex/skills/rough-cut/scripts/transcribe.sh <job_dir>
```
...
```json
{
  "segments": [
    ...
    { "clip": "clip-02.mov", "start": 4.95, "end": 8.10, "transcript": "to do shit Codex can do for free in five minutes" },
...
```bash
bash .Codex/skills/rough-cut/scripts/splice.sh <job_dir>
```
...
2. [clip-02 @ 4.95-8.10] "to do shit Codex can do for free"
...
```
...
- **Whisper can mishear.** Always cross-check the transcript before killing a line — sometimes "Codex" becomes "cloud" or "Cloud" and the line looks wrong when it's fine.
