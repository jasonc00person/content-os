---
name: captions
description: "Burns word-grouped captions onto a final video. Re-transcribes via WhisperX, Codex picks emphasis words from voice-dna, then generates a libass .ass with word-by-word reveal and burns via ffmpeg-full's ass filter. Three style presets: clean (default — minimal Helvetica), punch (viral yellow pop), mono (technical monospace). Triggers: add captions, caption this, burn captions, captions, subtitles."
---
...
1. Re-transcribe the final video:
   bash .Codex/skills/rough-cut/scripts/transcribe.sh /tmp/captions-<job>
   (after first copying the final mp4 into /tmp/captions-<job>/clip.mp4)

2. Codex reads /tmp/video-editor/captions-<job>/words.json + voice-dna.md
   and writes /tmp/video-editor/captions-<job>/emphasis.json with words to pop:
   {"emphasis": ["delete", "paying", "Codex", "five minutes"]}

3. Burn:
   bash .Codex/skills/captions/scripts/burn.sh <input.mp4> <words.json> [emphasis.json]
```
...
Without an emphasis.json, falls back to: **longest content word per line** (skipping common stopwords). Decent baseline, beatable by Codex reading voice-dna.
...
```bash
# Default — clean style with Codex-curated emphasis
bash .Codex/skills/captions/scripts/burn.sh \
...
bash .Codex/skills/captions/scripts/burn.sh ... --style punch
bash .Codex/skills/captions/scripts/burn.sh ... --style mono
